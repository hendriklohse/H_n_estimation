...
def fill_data(N, N_step, Nruns ,Nruns_step, est, ple):
	...
	dat_hill = [[None] * len(cols) for _ in range(len(rows))] #distance between estimate and real value of gamma(gamma=beta)
	...
	for i in range(len(rows)):
		for j in range(len(cols)):
			dat_hill[i][j] = run_all(n=n, nruns=nruns, ple=ple)[est[0]]
			dat_hill[i][j] = run_all(n=n, nruns=nruns, ple=ple)[est[1]]
			dat_hill[i][j] = run_all(n=n, nruns=nruns, ple=ple)[est[2]]
			n += N_step
			nruns += Nruns_step
...
fill_data(N=100000, N_step=50000, Nruns=1000000, Nruns_step=500000, est=["hill_ple", "moments_ple", "kernel_ple"], ple=2.5)
